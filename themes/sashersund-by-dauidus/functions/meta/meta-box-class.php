...
class Blogphix_meta_box {
	...
	function js_css() {
		...
		wp_enqueue_script('Blogphix-meta-box', $base_url . 'meta-box.css');
		wp_enqueue_script('Blogphix-meta-box', $base_url . 'meta-box.js', array('jquery'), null, true);
	}
	...
	function insert_images() {
		if (!isset($_POST['Blogphix-insert']) || empty($_POST['attachments'])) return;
		...
		foreach ($_POST['attachments'] as $attachment_id => $attachment) {
			...
			$li .= "<a title='" . __('Delete this image', 'Blogphix') . "' class='Blogphix-delete-file' href='#' rel='$nonce|$post_id|$id|$attachment_id'>" . __('Delete', 'Blogphix') . "</a>";
	...
	function check_field_color() {
		if ($this->has_field('color') && $this->is_edit_page()) {
			wp_enqueue_script('farbtastic');												// enqueue built-in script and style for color picker
	...
	function check_field_date() {
		if ($this->has_field('date') && $this->is_edit_page()) {
			...
			wp_enqueue_script('Blogphix-jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/themes/base/jquery-ui.css');
			wp_enqueue_script('Blogphix-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array('jquery'));
		}
	}
	...
	function check_field_time() {
		if ($this->has_field('time') && $this->is_edit_page()) {
			...
			wp_enqueue_script('Blogphix-jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/themes/base/jquery-ui.css');
			wp_enqueue_script('Blogphix-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array('jquery'));
			wp_enqueue_script('Blogphix-timepicker', 'https://github.com/trentrichardson/jQuery-Timepicker-Addon/raw/master/jquery-ui-timepicker-addon.js', array('Blogphix-jquery-ui'));
		}
	}
...
function check_field_single_image() {
	...
	function show() {
		...
		wp_nonce_field(basename(__FILE__), 'Blogphix_meta_box_nonce');
	...
	function show_field_begin($field, $meta) {
		echo "<th class='Blogphix-label-td'><label class='Blogphix-label' for='{$field['id']}'>{$field['name']}</label></th><td class='Blogphix-field' style='position: relative;'>";
	}
	...
	function show_field_text($field, $meta) {
		...
		echo "<input type='text' class='Blogphix-text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}

	function show_field_textarea($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<textarea class='Blogphix-textarea large-text' name='{$field['id']}' id='{$field['id']}' cols='60' rows='5'>$meta</textarea>";
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_select($field, $meta) {
		...
		$this->show_field_begin($field, $meta);
		echo "<select class='Blogphix-select' name='{$field['id']}" . ($field['multiple'] ? "[]' id='{$field['id']}' multiple='multiple'" : "'") . ">";
	...
	function show_field_radio($field, $meta) {
		$this->show_field_begin($field, $meta);
		foreach ($field['options'] as $key => $value) {
			echo "<input type='radio' class='Blogphix-radio' name='{$field['id']}' value='$key'" . checked($meta, $key, false) . " /> $value ";
		}
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_checkbox($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='checkbox' class='Blogphix-checkbox' name='{$field['id']}' id='{$field['id']}'" . checked(!empty($meta), true, false) . " /> {$field['desc']}</td>";
	}

	function show_field_wysiwyg($field, $meta) {
		$this->show_field_begin($field, $meta);
		//wp_tiny_mce(true, array('editor_selector' => 'Blogphix-rich-editor', 'remove_linebreaks' => false) );
		echo "<textarea class='Blogphix-wysiwyg Blogphix-rich-editor large-text' name='{$field['id']}' id='{$field['id']}' cols='60' rows='10'>$meta</textarea>";
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_file($field, $meta) {
		...
		if (!empty($meta)) {
			$nonce = wp_create_nonce('rw_ajax_delete');
			echo '<div style="margin-bottom: 10px"><strong>' . __('Uploaded files', 'Blogphix') . '</strong></div>';
			echo '<ol class="Blogphix-upload">';
			foreach ($meta as $att) {
				// if (wp_attachment_is_image($att)) continue; // what's image uploader for?
				echo "<li>" . wp_get_attachment_link($att, '' , false, false, ' ') . " (<a class='Blogphix-delete-file' href='#' rel='$nonce|{$post->ID}|{$field['id']}|$att'>" . __('Delete', 'Blogphix') . "</a>)</li>";
			}
			echo '</ol>';
		}

		// show form upload
		echo "<div style='clear: both'><strong>" . __('Upload new files', 'Blogphix') . "</strong></div>
			<div class='new-files'>
				<div class='file-input'><input type='file' name='{$field['id']}[]' /></div>
				<a class='Blogphix-add-file' href='#'>" . __('Add more file', 'Blogphix') . "</a>
			</div>
		</td>";
	}
	...
	function show_field_image($field, $meta) {
		...
		echo "<input type='hidden' class='Blogphix-images-data' value='{$post->ID}|{$field['id']}|$nonce_sort' />
			  <ul class='Blogphix-images Blogphix-upload' id='Blogphix-images-{$field['id']}'>";
		...
		foreach ($images as $image) {
			...
			echo "<li id='item_$image'>
					<img src='$src' />
					<a title='" . __('Delete this image', 'Blogphix') . "' class='Blogphix-delete-file' href='#' rel='$nonce_delete|{$post->ID}|{$field['id']}|$image'>" . __('Delete', 'Blogphix') . "</a>
					<input type='hidden' name='{$field['id']}[]' value='$image' />
				</li>";
		}
		echo '</ul>';

		echo "<a href='#' class='Blogphix-upload-button button' rel='{$post->ID}|{$field['id']}'>" . __('Add more images', 'Blogphix') . "</a>";
	}

	function show_field_color($field, $meta) {
		if (empty($meta)) $meta = '#';
		$this->show_field_begin($field, $meta);
		echo "<input class='Blogphix-color' type='text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='8' />
			  <a href='#' class='Blogphix-color-select' rel='{$field['id']}'>" . __('Select a color', 'Blogphix') . "</a>
			  <a href='#' class='Blogphix-color-select' style='display: none;'>" . __('Hide Picker', 'Blogphix') . "</a>
			  <div style='display:none; position: absolute; background: #f0f0f0; border: 1px solid #ccc; z-index: 100;' class='Blogphix-color-picker' rel='{$field['id']}'></div>";
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_checkbox_list($field, $meta) {
		...
		$html = array();
		foreach ($field['options'] as $key => $value) {
			$html[] = "<input type='checkbox' class='Blogphix-checkbox_list' name='{$field['id']}[]' value='$key'" . checked(in_array($key, $meta), true, false) . " /> $value";
		}
	...
	function show_field_date($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='Blogphix-date' name='{$field['id']}' id='{$field['id']}' rel='{$field['format']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}

	function show_field_time($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='Blogphix-time' name='{$field['id']}' id='{$field['id']}' rel='{$field['format']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}

	function show_field_single_image($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='Blogphix-single-image' name='{$field['id']}' id='{$field['id']}' value='$meta' size='18' />";
		echo "<input type='button' class='Blogphix-upload-single-image button-secondary' value='Choose Image'/><br />";
		$this->show_field_end($field, $meta);
	}
	
	function show_field_slider($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<div class='Blogphix-slider' rel='{$field['id']}'></div>";		
	...
	function save($post_id) {
		...
		|| (!check_admin_referer(basename(__FILE__), 'Blogphix_meta_box_nonce'))		// verify nonce
		...
		foreach ($this->_fields as $field) {
			...
			// validate meta value
			if (class_exists('Blogphix_meta_box_Validate') && method_exists('Blogphix_meta_box_Validate', $field['validate_func'])) {
				$new = call_user_func(array('Blogphix_meta_box_Validate', $field['validate_func']), $new);
			}
