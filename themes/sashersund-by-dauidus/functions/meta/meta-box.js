jQuery(document).ready(function($) {
	...
	// colorpicker field
	$('.Blogphix-color-picker').each(function(){
		var $this = $(this),
			id = $this.attr('rel');

		$this.farbtastic('#' + id);
	});
	$('.Blogphix-color-select').click(function(){
		$parent = $(this).parent();
		$(this).siblings('.Blogphix-color-picker').toggle();
		$('.Blogphix-color-select', $parent).toggle();
		return false;
	});

	// add more file
	$('.Blogphix-add-file').click(function(){
	...
	// delete file
	$('.Blogphix-upload').delegate('.Blogphix-delete-file', 'click' , function(){
		var $this = $(this),
			$parent = $this.parent(),
			data = $this.attr('rel');
	...
	$('.Blogphix-images').each(function(){
		...
		$this.sortable({
			...
			update: function (){
				...
				data = order + '|' + $this.siblings('.Blogphix-images-data').val();
	...
	// thickbox upload
	$('.Blogphix-upload-button').click(function(){
	...
	// thickbox upload
    window.formfield = '';
    $('.Blogphix-upload-single-image').live('click', function() {
        window.formfield = $('.Blogphix-single-image',$(this).parent());
        tb_show('', 'media-upload.php?type=image&TB_iframe=true');
        return false;
    });
    window.Blogphix_original_send_to_editor = window.send_to_editor;
    window.send_to_editor = function(html) {
        if (window.formfield) {
            ...
            window.Blogphix_original_send_to_editor(html);
        }
        window.formfield = '';
