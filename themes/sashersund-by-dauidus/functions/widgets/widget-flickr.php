...
class Blogphix_flickr extends WP_Widget {
	
	function Blogphix_flickr() {
		
		// define widget title and description
		$widget_ops = array('classname' => 'Blogphix_flickr_widget',
							'description' => __( 'Pulls in images from your Flickr account.', 'text_domain') );
		// register the widget
		$this->WP_Widget('Blogphix_flickr', __('Blogphix - Flickr', 'text_domain'), $widget_ops);
	...
	function widget( $args, $instance ) {
		...
		echo $before_widget;
             if ( $title )
				...
				<ul class="flickrImages">
					<li class="flickr_img_list"><script type="text/javascript" src="http://www.flickr.com/badge_code_v2.gne?count=<?php echo $number; ?>&amp;display=latest&amp;size=s&amp;layout=x&amp;source=user&amp;user=<?php echo $id; ?>" ></script></li>
				</ul>
			<?php
	...
	<?php _e('Title:', 'text_domain'); ?></label>
	...
	<?php _e('Flickr ID ', 'text_domain'); ?>(<a href="http://www.idgettr.com" target="_blank">idGettr</a>):</label>
	...
	<?php _e('Images to show:', 'text_domain'); ?></label>
...
add_action('widgets_init', create_function('', 'return register_widget("Blogphix_flickr");'));	
?>