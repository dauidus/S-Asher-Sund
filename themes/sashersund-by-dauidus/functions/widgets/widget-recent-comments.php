...
function _comments($no_comments = 4, $comment_len = 65) {
	global $wpdb;
		...
		foreach ($comments as $comment) {
			...
			?>	
                ...
                <li class="side-comment-wrap clearfix">
                    ...
                    <a href="<?php echo get_permalink( $comment->comment_post_ID ) . '#comment-' . $comment->comment_ID; ?>">View post</a> 
...
function Blogphix_get_author($comment) {
	...
	if ( empty($comment->comment_author) )
		$author = __('Anonymous', 'text_domain');
	else
