...
class ONetRegenThumbs {
	...
	**/
	public function __construct() {
		...
		$default_opts = array(
			...
			"succes_redir" => false,            // Redirect user after success process.
	...
	**/
	...
	global $current_screen;
		...
		wp_localize_script("onet-regenthumb-main", "onetrt_opts", (array)$this->opts );
		...
		// Script for post editor/lister
			...
			$lang = array(
				...
				"media_btn" => $this->opts->after_media_btn ? 1 : 0,
				"bulk_action" => $this->opts->bulk_action ? 1 : 0,
	...
	**/
	...
	global $post;
		if (!current_user_can( $this->cap) || !$this->opts->after_media_btn) return;
	...
	**/
	function image_full_quality($quality) {
		if ($this->opts->quality_overwrite == true) {
	...
	**/ 
	...
	global $wpdb, $_wp_additional_image_sizes, $onetrt_edit_thumb, $onetrt_admin_meta_boxes, $onetrt_dashboard_vars;
		...
		if (isset($_POST['save_thumb']) || isset($_POST['update_thumb'])) {
			...
			if (!empty($_POST['ename'])) unset($this->opts->thumbs[$_POST['ename']]); // remove previous instance if ename is exists
			$this->opts->thumbs[$thumb_meta['name']] = $thumb_meta;
			update_option("onetrt_opts",(array)$this->opts);
		...
		if (isset($_GET['thumb_remove']) && isset($this->opts->thumbs[$_GET['thumb_remove']])) {
			unset($this->opts->thumbs[$_GET['thumb_remove']]);
			update_option("onetrt_opts",(array)$this->opts);
			$custom_dims--;
		...
		if (!empty($_GET['thumb_edit']) && !empty($this->opts->thumbs[sanitize_title_with_dashes($_GET['thumb_edit'])]) ) {
			$onetrt_edit_thumb = array_merge($onetrt_edit_thumb,$this->opts->thumbs[sanitize_title_with_dashes($_GET['thumb_edit'])]);
		}
		...
		if (isset($_POST['save_settings'])) {
			$new_opts = array(
				...
				"thumbs" =>              $this->opts->thumbs
				);
			update_option("onetrt_opts",$new_opts);
			$this->opts = (object)$new_opts;
		...
		# Load items
			...
			if (($task = $this->get_task((int)$_GET['task_id'])) != "no_task") {
				...
				if (!isset($_GET['status_update'])) {
					...
					while ($doNext == true) {
						if ($this->opts->only_complement == true) :
						...
						if ($this->opts->massgen != true) $doNext = false;
