	...
	**/
	runProcess = function () {
		...
		makeTimeLive = function () {
			var time = parseInt(estTimeElem.html());
			if (time > 0 && !terminated) estTimeElem.html( humanTime(time-1) );
		}
		...
		updateProgress = function () {
			...
			estTimeElem.html( humanTime(remaining) );
			remainingElem.html(item_num-doneItems);
			percentElem.html( Math.ceil(percent) + "%");
			progressbarElem.progressbar( "option", "value", percent );
		}
